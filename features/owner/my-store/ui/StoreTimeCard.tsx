...
import { Clock3, ChevronRight, Edit3, Save, Power, AlertCircle, X } from 'lucide-react'
        ...
        <>
            <article className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm transition-all hover:shadow-md">
                ...
                <div className="mt-4 flex flex-col items-center gap-4 lg:flex-row">
                    ...
                    >
                        {isEditing ? (
                            <div className='flex flex-col gap-4 p-3 bg-gray-50 rounded-2xl'>
                                <Form
                                    form={form}
                                    layout="vertical"
                                    onFinish={handleSave}
                                    className="rounded-2xl bg-gray-50 w-full!"
                                >
                                    <div className="flex gap-2 w-full">
                                        <Form.Item name="open" label={<span className=" text-blue-500 uppercase tracking-tight font-bold text-center">Open</span>} rules={[{ required: true, message: '오픈 시간을 선택해주세요' }]}
                                            labelCol={{ style: { width: '100%', display: 'flex', justifyContent: 'center' } }}
                                            className='flex-1 text-blue-500!'
                                        >
                                            <TimePicker format="HH:mm" className="w-full h-11 rounded-xl" />
                                        </Form.Item>
                                        <Form.Item name="close" label={<span className="text-orange-500 uppercase tracking-tight font-bold text-center">Close</span>} rules={[{ required: true, message: '마감 시간을 선택해주세요' }]}
                                            labelCol={{ style: { width: '100%', display: 'flex', justifyContent: 'center' } }}
                                            className='flex-1'
                                        >
                                            <TimePicker format="HH:mm" className="w-full h-11 rounded-xl" />
                                    ...
                                    <div className="flex gap-2 items-center">
                                        ...
                                        <Button
                                            ...
                                            block
                                            icon={<Save size={18} />}
                                            className="h-12! flex-1! rounded-xl! bg-emerald-500! hover:bg-emerald-700!"
                                        >
                                            저장하기
                                        </Button>
                                    </div>
                            ...
                            <div className="group transition-all hover:bg-gray-100 cursor-pointer w-full relative flex items-center gap-3 rounded-2xl bg-gray-50 p-3 ">
                    ...
                    <div className="flex w-full flex-1 gap-2">
                        <button
                            type="button"
                            className="cursor-pointer flex flex-1 items-center justify-center gap-2 rounded-xl border border-rose-100 bg-white py-2.5 text-sm font-bold text-rose-500 transition hover:bg-rose-50 active:scale-95"
                        >
                            <Power size={14} />
                            오늘 즉시 휴무
                        </button>
                        ...
                        <button
                            type="button"
                            className="cursor-pointer flex flex-1 items-center justify-center gap-2 rounded-xl border border-amber-100 bg-white py-2.5 text-sm font-bold text-amber-600 transition hover:bg-amber-50 active:scale-95"
                        >
                            <AlertCircle size={14} />
                            조기 마감
                        </button>
