    ...
    return (
        <Form form={form} layout="vertical" onFinish={add} onFinishFailed={(error) => console.error('검증 실패:', error)} className="flex flex-col h-full">
            <section className="space-y-2 flex-1 overflow-y-auto pb-4 ">
                <Form.Item
                    name="category_id"
                    label={<span className="text-[13px] font-bold text-slate-500 ml-1 flex items-center gap-1"><LayoutGrid size={14} className="text-purple-400" /> 카테고리</span>}
                >
                    <Select
                            ...
                            label: (
                                <div className="flex items-center justify-between group">
                                    ...
                                    <Button
                                        type="text"
                                        size="small"
                                        className="opacity-0 group-hover:opacity-100 p-0 h-6 w-6 flex items-center justify-center rounded-full hover:bg-red-50! transition-all"
                            ...
                            <div onMouseDown={(e) => e.preventDefault()}>
                                ...
                                <Space.Compact className="w-full px-1 pb-1">
                                    ...
                                    <Button
                                        ...
                                        className="h-10! rounded-r-xl! bg-emerald-500! border-none!"
                                    >
                                        추가
                                    </Button>
                                </Space.Compact>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <Form.Item name="duration_minutes" label={<span className="text-[13px] font-bold text-slate-500 ml-1 flex items-center gap-1"><AlarmClockCheck size={14} className="text-blue-400" /> 기본 시간(분)</span>}>
