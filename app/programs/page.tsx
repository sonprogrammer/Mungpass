...
import { MapPin, Calendar, Star, X, Info, CheckCircle, Share2 } from 'lucide-react';
// 임시 데이터 (추후 Supabase 연동)
const PROGRAMS_DATA = [
  {
    id: 1,
...
export default function Programs() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* 3. 상세 정보 오버레이 (Mock-up) */}
      {/* 3. 상세 정보 오버레이 (배경 클릭 시 닫기 추가) */}
{selectedProgram && (
  <div 
    // 바깥 어두운 배경 클릭 시 setSelectedProgram(null) 호출
    onClick={() => setSelectedProgram(null)}
    className="fixed inset-0 z-[100] flex items-end justify-center bg-black/40 backdrop-blur-sm transition-all"
  >
    <div 
      // 버블링(이벤트 전파) 방지: 내용물 클릭 시에는 배경의 onClick이 실행되지 않음
      onClick={(e) => e.stopPropagation()}
      className="w-full max-w-[480px] bg-white rounded-t-[40px] shadow-2xl animate-in slide-in-from-bottom duration-300"
    >
      {/* 핸들바 (시각적으로 끌어내릴 수 있음을 암시) */}
